...
import { AiOutlineStar, AiOutlineUser } from 'react-icons/all'


const Course: FC<ICourse> = ({ image, category, title, participants, rating, price }) => {
 return (
    <div className="p-3 shadow-lg min-w-[15rem] bg-white rounded-md">
      <img src={image} alt="" />
      <div className="mt-2 text-xs text-Teal">{category}</div>
      <div className="text-sm mt-2 font-bold text-gray">{title}</div>
